...
var PagingCallback
function InitTable(t, url, pageSize, showEmptyHeader, fn, where0, sort0) {
    //if (fn != null)
    // PagingCallback = fn
    t.data('callback', fn)
...
function LoadTable(t, sort, where, showEmptyHeader) {
    ...
    if (t.find('th').length) {
        ...
        $('<table id=tExcel width=100%><tr><td><img style="float:right;margin-left:5px" src=/images/excel.png /></td></tr></table>').insertBefore(t).find('img').on('click', function () {
...
function AddKendoDropDownList(src, arr, fn) {
    var json = []
    for (var i in arr)
        json.push({ text: arr[i], value: arr[i] })
...
function AddPagingFooter(pageNo, pageSize, total, t) {
    var html = '<div class="k-pager-wrap k-grid-pager k-widget k-floatwrap" data-role="pager">'
    var limit = 10
...
function GetButton(name) {
    return '<input style="margin-left:5px" type=button class="btn btn-common" value=' + name + ' onclick=on_Action("' + name + '") />'
}
function GetImg(name, img) {
    return '<img value=' + name + ' style="margin-left:10px" src="/Images/' + img + '"  onclick=on_Action("' + name + '") />'
}
...
function InitTableNoPaging(t, url, obj, showHeader, json0) {
    ...
    t.width('100%').attr('border', 0).addClass('k-grid')
    t.html(html)
    AddLeftBorder()
    if (html == '' && showHeader)
