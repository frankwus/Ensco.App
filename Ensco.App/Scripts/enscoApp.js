...
$(document).ready(function () {
    ...
    $(document).on('mouseover', 'img', function (e) {
        return 
        var src = $(this)
        src.removeClass() //'dxGridView_CTExportCsvHover_EnscoTheme')
        e.preventDefault() 
        e.stopPropagation() 
        console.log(src[0].outerHTML  )
        return false 
    })
})
function ShowAlert() {
   return 
    try {
        if (typeof PopupAlert === "undefined")
            return
        var g = grid_HomeTasksTask1
        g.GetVisibleRowsOnPage()
        if (g.GetVisibleRowsOnPage() < 1 || !g.IsVisible())
            g.Refresh()

    } catch (e) {
    }
}
function onPopupControlClose(s, e) {
    // s.Show() 
}
function onEndCallbackIrmaHomeEnscoPassportTasksJobPartial(s, e) {    
...
function ShowFrame() {
    var src = GetId('popupJobsHome_PW-1')
...
function GetAjaxArr(url, obj, raw) {
    var r
    url = GetSiteUrl() + url
    var para = ''
    if (obj != null) {
...
function LoadTable(t, sort, where, showEmptyHeader) {
    ...
    if (t.find('th').length) {
        ...
        $('<table id=tExcel class="kgrid-custom-toolbar" width=100%><tr><td><img style="float:right;margin-left:5px" src=/images/excel.png /></td></tr></table>').insertBefore(t).find('img').on('click', function () {
...
function InitLang() {
    ...
    if (Lang == null || (Lang == 'EN' && !LangClicked))
        return
    var arr = ['label, td.header', 'a:not(".nolang")', 'td.tlc-header-column', 'input.btn-common']
...
function InitType() {
    $.ajaxSetup({
        beforeSend: function (jqXHR, settings) {
            ...
            var arr = data.split('&')
            for (var i in arr) {
                if (arr[i].toLowerCase().indexOf('type=') == 0)
                    arr[i] = 'Type=' + Type
            }
...
function on_PDF(page, id, lang ) {
    //SetTabIndex()
    window.open('/common/common/pdf?page=' + page + '&id=' + id + '&lang=' + lang, '_blank'     );
